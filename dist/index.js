...
var hexToCssFilter = require('hex-to-css-filter');
...
function compute(input, acceptanceLossPercentage) {
    if (isHEXValid(input)) {
        return hexToCssFilter.hexToCSSFilter(input, config);
    }
    else if (isRGBValid(input)) {
        return hexToCssFilter.hexToCSSFilter(rgbToHex(input), { ...config, acceptanceLossPercentage: acceptanceLossPercentage || 1 });
    }
    else {
        return { filter: input };
    }
